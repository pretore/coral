...
static void check_iterator_alloc_error_on_null_argument_ptr(void **state) {
    coral_error = CORAL_ERROR_NONE;
    assert_false(coral$tree_map$iterator_alloc(NULL));
    assert_int_equal(CORAL_ERROR_ARGUMENT_PTR_IS_NULL, coral_error);
    coral_error = CORAL_ERROR_NONE;
}
...
static void check_iterator_init_error_on_null_object_ptr(void **state) {
    coral_error = CORAL_ERROR_NONE;
    assert_false(coral$tree_map$iterator_init(NULL, (void *)1));
    assert_int_equal(CORAL_ERROR_OBJECT_PTR_IS_NULL, coral_error);
    coral_error = CORAL_ERROR_NONE;
}
...
static void check_iterator_init_error_on_null_argument_ptr(void **state) {
    coral_error = CORAL_ERROR_NONE;
    assert_false(coral$tree_map$iterator_init((void *)1, NULL));
    assert_int_equal(CORAL_ERROR_ARGUMENT_PTR_IS_NULL, coral_error);
    coral_error = CORAL_ERROR_NONE;
}
...
static void check_iterator_init(void **state) {
    ...
    assert_true(coral$tree_map$init(
            object, sizeof(size_t), sizeof(size_t), coral_compare_size_t));
    struct coral$tree_map$iterator it;
    assert_true(coral$tree_map$iterator_init(&it, object));
    assert_ptr_equal(it.iterator.tree_set, &object->tree_set);
    assert_true(coral$tree_map$invalidate(object, NULL));
    free(object);
    coral_error = CORAL_ERROR_NONE;
}
...
static void check_iterator_get_next_error_on_null_object_ptr(void **state) {
    coral_error = CORAL_ERROR_NONE;
    assert_false(coral$tree_map$iterator_get_next(NULL, (void *)1));
    assert_int_equal(CORAL_ERROR_OBJECT_PTR_IS_NULL, coral_error);
    coral_error = CORAL_ERROR_NONE;
}

static void check_iterator_get_next_error_on_null_argument_ptr(void **state) {
    coral_error = CORAL_ERROR_NONE;
    assert_false(coral$tree_map$iterator_get_next((void *)1, NULL));
    assert_int_equal(CORAL_ERROR_ARGUMENT_PTR_IS_NULL, coral_error);
    coral_error = CORAL_ERROR_NONE;
}
...
static void check_iterator_get_next_error_on_end_of_sequence(void **state) {
    ...
    assert_true(coral$tree_map$init(
            object, sizeof(size_t), sizeof(size_t), coral_compare_size_t));
    struct coral$tree_map$iterator iterator = {};
    assert_true(coral$tree_map$iterator_init(&iterator, object));
    struct entry {
        size_t key;
        size_t value;
    } e;
    assert_false(coral$tree_map$iterator_get_next(&iterator, (void **) &e));
    assert_int_equal(CORAL_ERROR_END_OF_SEQUENCE, coral_error);
    assert_true(coral$tree_map$invalidate(object, NULL));
    free(object);
    coral_error = CORAL_ERROR_NONE;
...
static void check_iterator_get_next_error_on_object_unavailable(void **state) {
    ...
    struct coral$tree_map$iterator iterator = {};
    assert_true(coral$tree_map$iterator_init(&iterator, object));
    assert_true(coral$tree_map$delete(object, &e));
    assert_false(coral$tree_map$iterator_get_next(&iterator, (void **) &e));
...
static void check_iterator_get_next(void **state) {
    ...
    assert_true(coral$tree_map$insert(object, &e));
    struct entry d = {1, 28232};
    assert_true(coral$tree_map$insert(object, &d));
    struct coral$tree_map$iterator iterator = {};
    assert_true(coral$tree_map$iterator_init(&iterator, object));
    struct entry f;
    assert_true(coral$tree_map$iterator_get_next(&iterator, (void **) &f));
    assert_memory_equal(&f, &d, sizeof(struct entry));
    assert_true(coral$tree_map$iterator_get_next(&iterator, (void **) &f));
    assert_memory_equal(&f, &e, sizeof(struct entry));
    assert_false(coral$tree_map$iterator_get_next(&iterator, (void **) &f));
    assert_int_equal(CORAL_ERROR_END_OF_SEQUENCE, coral_error);
    assert_true(coral$tree_map$invalidate(object, NULL));
    free(object);
    coral_error = CORAL_ERROR_NONE;
...
static void check_iterator_delete_error_on_null_object_ptr(void **state) {
    coral_error = CORAL_ERROR_NONE;
    assert_false(coral$tree_map$iterator_delete(NULL));
    assert_int_equal(CORAL_ERROR_OBJECT_PTR_IS_NULL, coral_error);
    coral_error = CORAL_ERROR_NONE;
}

static void check_iterator_delete_error_on_invalid_value(void **state) {
    coral_error = CORAL_ERROR_NONE;
    struct coral$tree_map$iterator iterator = {};
    assert_false(coral$tree_map$iterator_delete(&iterator));
    assert_int_equal(CORAL_ERROR_INVALID_VALUE, coral_error);
    coral_error = CORAL_ERROR_NONE;
}

static void check_iterator_delete_error_on_object_unavailable(void **state) {
    coral_error = CORAL_ERROR_NONE;
    struct coral$tree_map *object;
    assert_true(coral$tree_map$alloc(&object));
    assert_true(coral$tree_map$init(
...
static void check_iterator_delete(void **state) {
    coral_error = CORAL_ERROR_NONE;
    struct coral$tree_map *object;
    assert_true(coral$tree_map$alloc(&object));
    assert_true(coral$tree_map$init(
    ...
    struct coral$tree_map$iterator iterator = {};
    assert_true(coral$tree_map$iterator_init(&iterator, object));
    assert_memory_equal(&e, object->tree_set.tree.root, sizeof(struct entry));
    assert_true(coral$tree_map$iterator_delete(&iterator));
    assert_null(object->tree_set.tree.root);
    assert_null(iterator.iterator.data);
    assert_int_equal(2, object->tree_set.id);
    assert_int_equal(2, iterator.iterator.id);
    assert_true(coral$tree_map$invalidate(object, NULL));
    free(object);
    coral_error = CORAL_ERROR_NONE;
}
...
static void check_iterator_invalidate_error_on_object_null_ptr(void **state) {
    coral_error = CORAL_ERROR_NONE;
    assert_false(coral$tree_map$iterator_invalidate(NULL));
    assert_int_equal(CORAL_ERROR_OBJECT_PTR_IS_NULL, coral_error);
    coral_error = CORAL_ERROR_NONE;
}
...
static void check_iterator_invalidate(void **state) {
    ...
    assert_true(coral$tree_map$iterator_invalidate(&iterator));
    assert_int_equal(iterator.iterator.id, 0);
    assert_int_equal(iterator.iterator.data, 0);
    coral_error = CORAL_ERROR_NONE;
}
...
int main(int argc, char *argv[]) {
    const struct CMUnitTest tests[] = {
            ...
            cmocka_unit_test(check_iterator_get_next_error_on_null_argument_ptr),
