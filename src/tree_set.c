...
bool coral$tree_set$insert(struct coral$tree_set *object, const void *item) {
    ...
    void *node;
    if (!coral$red_black_tree$node_of(sizeof(void *), &node)) {
        return false;
    }
...
static void
coral$tree_set$delete_item(struct coral$tree_set *object, void* item) {
...
bool coral$tree_set$iterator_next(struct coral$tree_set$iterator *object,
    ...
    memcpy(out, object->data, sizeof(void *));
